...
- **贴边缩球**：把卡片拖到屏幕左 / 右边缘（拖过边缘也算）松手，卡片以
  缓出动画平滑缩成小球并吸附贴边。小球为「极光玻璃珠」形态：磨砂玻璃底 +
  青→紫→粉极光渐变进度环（统一配色，不随用量变色）+ 漂浮微粒与环绕星点，
  两位小数显示总量已用百分比；单击小球或把它拖离边缘即动画展开回卡片
- 卡片三环用量着色：< 50% 绿 / < 80% 黄 / ≥ 80% 红，附重置倒计时，
  百分比两位小数显示（吸附球进度环为统一极光渐变色）
- 显示 Kimi Code 用量占比
